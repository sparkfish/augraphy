...
class InkGenerator:
    ...
    def generate_noise_clusters(self, image, n_clusters=(200,200), n_samples=(300,300), std_range=(5,10)):
        ...
        generated_points_x = np.delete(generated_points[:, 0], ind_delete.reshape(ind_delete.shape[0]), axis=0).astype("int")
        generated_points_y = np.delete(generated_points[:, 1], ind_delete.reshape(ind_delete.shape[0]), axis=0).astype("int")
    ...
    def generate_lines(self, ink_background):
        ...
        for i in range(ink_draw_iterations):
            ...
            if self.ink_type == "highlighter":
                lines_background = cv2.addWeighted(lines_background, 0.5, np.full_like(lines_background, fill_value=255, dtype="uint8"), 0.5, 0)
        ...
        if self.ink_skeletonize:
            ...
            max_iter = random.randint(self.ink_skeletonize_iterations_range[0], self.ink_skeletonize_iterations_range[1])
    ...
    def generate_text(self, ink_background):
        ...
        for i in range(ink_draw_iterations):
            ...
            # reduce transparency for highligther
            if self.ink_type == "highlighter":
                texts_background = cv2.addWeighted(texts_background, 0.5, np.full_like(texts_background, fill_value=255, dtype="uint8"), 0.5, 0)

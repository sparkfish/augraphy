...
class PatternMaker:
    ...
    def superimpose(self, original_image, mesh_image):
        ...
        overlay_pattern = cv2.addWeighted(
            mesh_image, self.alpha, original_image, 1 - self.alpha, self.gamma)
        if self.display:
    ...
    def make_patterns(self, image, mesh_img, window_size = 200):
        ...
        ****************************************
        ...
        while True:
            ...
            if hy > 0 and wx  > 0 and initial_coords[0] + wx < w and initial_coords[1] + hy < h and initial_coords[0] + wx > 0 and initial_coords[1] + hy > 0:
                image[initial_coords[1]:initial_coords[1] + hy,  initial_coords[0]:initial_coords[0] + wx,:] = self.superimpose(img[initial_coords[1]:initial_coords[1] + hy,  initial_coords[0]:initial_coords[0] + wx,:], mesh_img)
            img = image.copy()

        ...
        if force or self.should_run():
            ...
            if self.reflected_light_color == "random":
                ...
                if reflected_light_color[0] < 1 or reflected_light_color[1] < 1 or reflected_light_color[0] < 1:
                    reflected_light_color = (max(1, reflected_light_color[0]), max(1, reflected_light_color[1]), max(1, reflected_light_color[2]))
            ...
            if self.reflected_light_internal_radius_range[1] <= 1 and isinstance(self.reflected_light_internal_radius_range[1], float):
                reflected_light_internal_radius_range = [self.reflected_light_internal_radius_range[0]*min_size, self.reflected_light_internal_radius_range[1]*min_size]  
            else:
                reflected_light_internal_radius_range = self.reflected_light_internal_radius_range
            if self.reflected_light_external_radius_range[1] <= 1 and isinstance(self.reflected_light_external_radius_range[1], float):
                reflected_light_external_radius_range = [self.reflected_light_external_radius_range[0]*min_size, self.reflected_light_external_radius_range[1]*min_size]
            else:
                reflected_light_external_radius_range = self.reflected_light_external_radius_range
            reflected_light_internal_radius = random.randint(int(reflected_light_internal_radius_range[0]), int(reflected_light_internal_radius_range[1]))
            reflected_light_external_radius = random.randint(int(reflected_light_external_radius_range[0]), int(reflected_light_external_radius_range[1]))
            ...
            reflected_light_internal_max_brightness = random.uniform(self.reflected_light_internal_max_brightness_range[0],self.reflected_light_internal_max_brightness_range[1])
            reflected_light_external_max_brightness = random.uniform(self.reflected_light_external_max_brightness_range[0],self.reflected_light_external_max_brightness_range[1])
            
            # reflected_light ellipse angle
            reflected_light_ellipse_angle = random.randint(self.reflected_light_ellipse_angle_range[0],self.reflected_light_ellipse_angle_range[1])

            # reflected_light gaussian kernel value
            reflected_light_gaussian_kernel_value = random.randint(self.reflected_light_gaussian_kernel_size_range[0], self.reflected_light_gaussian_kernel_size_range[1])
            # gaussian kernel value must be odd
            if not reflected_light_gaussian_kernel_value % 2:
                 reflected_light_gaussian_kernel_value += 1


            reflected_light_minor_major_ratio = random.uniform(self.reflected_light_minor_major_ratio_range[0], self.reflected_light_minor_major_ratio_range[1])

            # axes which is the major and minor radius of ellipse
            reflected_light_axes = [reflected_light_external_radius+reflected_light_internal_radius, int((reflected_light_external_radius+reflected_light_internal_radius)*reflected_light_minor_major_ratio)]
            ...
            while True:  
                ...
                # draw ellipse
                cv2.ellipse(image_background_new, (reflected_light_center_x, reflected_light_center_y), reflected_light_axes, reflected_light_ellipse_angle,
                          0, 360, reflected_light_color, step_length)
            ...
            image_background = cv2.GaussianBlur(image_background, (reflected_light_gaussian_kernel_value, reflected_light_gaussian_kernel_value), 0)

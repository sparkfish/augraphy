...
class VoronoiTessellation(Augmentation):
        ...
        for y in range(width):
            for x in range(height):
                ...
                for i in range(num_cells):
                    d = math.hypot((pixel_data[0][i]-x+perlin_noise_2d[0][x][y]), (pixel_data[1][i]-y+perlin_noise_2d[1][x][y]))
                    if d < dmin:
                            dmin = d
                            j = i
                    nsize[j]+=1
    ...
    def apply_augmentation(self):
        ...
        if self.perlin:
            perlin_x = np.array([[obj_noise.noise2D(x/100, y/100)*self.mult for y in range(self.height)] for x in range(self.width)])
            perlin_y = np.array([[obj_noise.noise2D((x+self.seed)/100, (y+self.seed)/100)*self.mult for y in range(self.height)] for x in range(self.width)])
        ...
        ng = [random.randrange(self.background_value[0], self.background_value[1]) for _ in range(self.num_cells)] # go through the number of cells and assign color
        nsize = np.zeros(self.num_cells, dtype=np.int32)
        img_array =  self.generate_voronoi(self.width, self.height,self.num_cells, nsize, (nx, ny, ng)  , (perlin_x, perlin_y))

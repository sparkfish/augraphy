...
class InkMottling(Augmentation):
    ...
    def __call__(self, image, layer=None, force=False):
        if force or self.should_run():
            ...
            ink_mottling_noise_scale = random.randint(self.ink_mottling_noise_scale_range[0], self.ink_mottling_noise_scale_range[1])
            image_random = np.random.randint(0,255,size=(int(ysize/ink_mottling_noise_scale), int(xsize/ink_mottling_noise_scale))).astype("uint8")
            image_random = cv2.cvtColor(image_random, cv2.COLOR_GRAY2BGR)
            
            # apply gaussian blur to the mask of noise
            kernel_value = random.randint(self.ink_mottling_gaussian_kernel_range[0], self.ink_mottling_gaussian_kernel_range[1])
             # kernel must be odd

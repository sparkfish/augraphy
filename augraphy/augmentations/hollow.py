    ...
    def __call__(self, image, layer=None, force=False):
        if force or self.should_run():
            ...
            for i in range(3):
                ...
                _, image_binary_single_channel = cv2.threshold(image[:,:,i], 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            ...
            # apply median filter
            median_kernel_value = upscale * random.randint(self.hollow_median_kernel_value_range[0], self.hollow_median_kernel_value_range[1])
            ...
            dilation_kernel_value = upscale * random.randint(self.hollow_dilation_kernel_size_range[0], self.hollow_dilation_kernel_size_range[1])

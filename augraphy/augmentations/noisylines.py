...
class NoisyLines(Augmentation):
    ...
    def draw_noisy_lines(self, image):
        ...
        for y_coordinate in y_coordinates:
            ...
            noisy_lines_thickness = random.randint(self.noisy_lines_thickness_range[0], self.noisy_lines_thickness_range[1])
            cv2.line(image_mask, (0, y_coordinate), (xsize-1, y_coordinate), self.noisy_lines_color, thickness=noisy_lines_thickness)
          
            noisy_lines_length_interval = random.randint(self.noisy_lines_length_interval_range[0], self.noisy_lines_length_interval_range[1])
                ...
                for new_y_coordinate in range(y_coordinate-half_thickness, y_coordinate+half_thickness+1):
                    if new_y_coordinate >= 0 and new_y_coordinate < ysize:
                        ...
                        for x in range(start_x, xsize,noisy_lines_length_interval*2):
                            image_mask[new_y_coordinate, x+noisy_lines_length_interval:x+(noisy_lines_length_interval*2)] = 255
        
        # apply noise in the line
        image_random = np.random.uniform(0, 1, size=image.shape[:2])
        noisy_lines_random_noise_intensity = random.uniform(self.noisy_lines_random_noise_intensity_range[0], self.noisy_lines_random_noise_intensity_range[1])
        image_mask[image_random<noisy_lines_random_noise_intensity] = 255

        # apply gaussian blur
        gaussian_kernel_value = random.randint(self.noisy_lines_gaussian_kernel_value_range[0], self.noisy_lines_gaussian_kernel_value_range[1])

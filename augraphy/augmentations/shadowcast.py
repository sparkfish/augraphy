    ...
    def __call__(self, image, layer=None, force=False):
        if force or self.should_run():
            ...
            for i in range(shadow_iterations):
                ...
                number_points = 2 + random.randint(max(1, self.shadow_vertices_range[0]), max(1, self.shadow_vertices_range[1]))
            ...
            shadow_mask_all = cv2.addWeighted(shadow_mask_all, shadow_opacity, np.full_like(image, fill_value=255, dtype="uint8"), 1-shadow_opacity, 0)

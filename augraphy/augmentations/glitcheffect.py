...
class GlitchEffect(Augmentation):
    ...
    def apply_glitch(self, image):
        ...
        for i in range(glitch_number):
            ...
            if self.glitch_size_range[0] <=1.0 and isinstance(self.glitch_size_range[0], float):
                glitch_size = random.randint(int(self.glitch_size_range[0]*ysize), int(self.glitch_size_range[1]*ysize))
            else:
            ...
            if self.glitch_offset_range[0] <=1.0 and isinstance(self.glitch_offset_range[0], float):
                glitch_offset = random.randint(int(self.glitch_offset_range[0]*xsize), int(self.glitch_offset_range[1]*xsize)) * direction
            else:
    ...
    def __call__(self, image, layer=None, force=False):
        if force or self.should_run():
            ...
            color_shift = ColorShift(color_shift_offset_x_range = (3,5),
